...
const lookup = new Map(PRODUCT_CATALOG.map((product) => [product.name.toLowerCase(), product.image]));

export function resolveProductImage(name: string, explicitImage = "") {
  const label = String(name || "").trim();
  return explicitImage || lookup.get(label.toLowerCase()) || "";
}

export function categoryOptions() {
  return ["General", "Local Greens", "Root Veg", "Allium", "Pods", "Fresh", "Spicy", "Leafy Greens"];
}
